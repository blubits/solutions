...
long long int count_rec(int i, int last_placed, bool has_l) {
    ...
    if (i == posns.size() - 1 && !has_l) {
        // printf("%d %d %d\n", type_r(curr_pos), type_c(curr_pos),
        // type_l(curr_pos));
        if (type_r(curr_pos) != IS_CONSONANT &&
            type_c(curr_pos) != IS_CONSONANT &&
            type_l(curr_pos) != IS_CONSONANT) {
            str[curr_pos] = 'L';
    ...
    // consonant
    if (type_r(curr_pos) != IS_CONSONANT && type_c(curr_pos) != IS_CONSONANT &&
        type_l(curr_pos) != IS_CONSONANT) {
        str[curr_pos] = 'B';
    ...
    // vowel
    if (type_r(curr_pos) != IS_VOWEL && type_c(curr_pos) != IS_VOWEL &&
        type_l(curr_pos) != IS_VOWEL) {
        str[curr_pos] = 'A';
...
long long int count() {
    if (posns.size() == 0) {
        ...
        for (int i = 0; i < len - 2; i++) {
            if (ltype(str[i]) + ltype(str[i + 1]) + ltype(str[i + 2]) == 30)
                return 0;
            if (ltype(str[i]) + ltype(str[i + 1]) + ltype(str[i + 2]) == 60)
                return 0;
        }
        return 1;
    }
